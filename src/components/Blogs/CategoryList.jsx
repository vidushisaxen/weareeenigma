...
const CategoryList = ({ categories, activeCategory, setActiveCategory }) => {
  ...
  const filteredCategories = categoriesArray.filter((category) => category.title !== 'Uncategorized');
  ...
  return (
    <div className='flex flex-wrap gap-6 tablet:gap-4 mobile:gap-2'>
      ...
      {filteredCategories.map((category) => (
        <Link key={category.slug.current} href={`/categories/${category.slug.current}`} className="block w-fit h-fit group" onClick={() => setActiveCategory(category.title)}>
          <div className={`px-[2vw] min-w-20 py-[0.5vw] text-black1 text-center relative w-fit border-[0.5px] border-current rounded-full tablet:py-[1vw] tablet:px-[3vw] overflow-hidden ${activeCategory === category.title ? 'bg-black text-white' : ''}`}>
